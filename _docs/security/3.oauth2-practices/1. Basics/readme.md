...
앞에서 살펴본 OAuth2의 주요 구성 요소들 사이의 OAuth2 프로토콜 플로우를 도식화하면 다음과 같다. 이 플로우는 OAuth2 프로토콜의 실제 구현 플로우 라기 보다는 구성 요소들의 역할과 상호작용, 그리고 핵심 개념을 설명하기 위한 다소 추상적인 개념의 플로우이다.
...
1. Authorization
	<p>
	OAuth2는 권한에 대한 인가(Authorization)에만 관심을 가진다. 그리고 자원 소유자가 클라이언트에게 승인한 권한 인가를 완벽히 보호 자원에게 보장하는 프로토콜이다.
	</p>
	
2.  Delegated Authorization
	<p>
	OAuth2는 자원 소유자의 자원 접근 권한을 클라이언트에게 위임(Delegate)하는 프로토콜이다. OAuth2는 이 위임 과정을 여러 인가 승인 타입(Authorization Grant Type)으로 상세한다. OAuth2 구현은 바로 이 인가 승인에 참여하고 있는 OAuth2 구성 요소와 그 과정을 구현하는 것이다.   
	</p>
	<p>
	한편, OAuth2는 자원 서버가 해야하는 승인된 인가 확인을 인가 서버에게 위임하게 한다.  따라서 OAuth2는 자원소유자/클라이언트 그리고 자원서버/인가서버 간에 신뢰 관계를 기본 전제로 한다. 그리고 이 둘 관계 사이에 있는 많은 보안 문제를 OAuth2는 단순한 방식으로 해결한다.  
	</p>
...
3. Access Token
	<p>
	OAuth2의 인가 승인 플로우에서 인가 승인의 물리적 증거는 액세스 토큰이다. 자원 소유자가 클라이언트에게 자원에 대한 인가 승인을 액세스 토큰 발급으로 보증한다. 인가 서버가 둘 사이에 위치하여 자원 소유자의 인가 승인을 액세스 토큰으로 클라이언트에게 발급하는 역할을 한다. 
	</p>
	<p>
	한편, 자원 서버는 클라이언트가 제출한 토큰을 받아 접근하려는 자원의 소유자가 권한 인가를 승인한 접근인지 판단해야 한다. 당연히 이는 토큰을 발급한 인가 서버가 확인해 준다.
	</p>

#### 3. Authorization Grant Type 
"OAuth2의 핵심은 자원 소유자의 자원에 대한 권한 <strong>인가 승인(Authorization Grant)</strong>, 이에 대한 <strong>액세스 토큰</strong> 보증, 그리고 보증된 토큰으로 자원 접근이다" 

이를 구현하기 위해 OAuth2는 인가 승인 타입(Authorization Grant Type)이라는 모델들을 상세하고 있다. 대부분의 표준 상세가 그렇지만 OAuth2도 모델만을 제시할 뿐 구체적인 기술들에 대한 상세는 없다. 따라서 구현에서도 플랫폼, 개발언어, 통신 방식도 자유롭다. 하지만 실제적으로는 OAuth2 구성 요소들 사이의 HTTP 통신으로 구현되며 구성 요소들은 소프트웨어 또는 구현 코드가 된다. 예제는 다음과 같은 기술들로 구성 요소들을 구현한다.

1. Authorization Server
	- JBoss Keycloak 솔류션
	- Implementation I - Authorization Server  참고 
2. Resource Server
	- Spring Web
	- Spring Security OAuth
	-  Implementation II - Resource Server 참고
3. Client
	- Spring Security OAuth
	- Implementation III - Client 참고
4. Access Token
	- JWT     

...
2.	URL를 기반한 컴포넌트 렌더링을 위해서는 이미 잘 구현된 React Router의 라우팅 기본 기능에 JWT Security Feature를 더 추가 구현해야 한다.
3. 구현 컴포넌트
	- &lt;AuthContextRouter&gt;
	- &lt;AuthRoutes&gt;
...
2.	이를 위해서는 Route Container 즉,  &lt;Routes&gt; 들을 nest 하고 동적으로 BrowserRouter를 생성해야 한다. (코드 참고)

#### 3. Nesting Route Component
1. Security Feature의 Cross-Cutting을 위해  &lt;Outlet&gt; 기반의 중첩(nested) &lt;Routes&gt;를 사용한다.
...
2.  즉, 접근제어 적용에 너무 많은 것을 알고 작성해야 하는 문제점이 있다. 기존에 지원하던 중첩 라우팅이 한 depth 늘어 복잡해 진다. 보안 관련 Nesting Route Component 들을 감추고 기존의 중첩 라우팅을 지원하기 위해 &lt;AuthRoute&gt; 라는 Route Container를 지원한다.
...
4. 구현을 위해서는 createBrowserRouter API를 사용해서 BrowserRouter를  코드로 직접 생성하고 RouterProvider component의 도움을 받아야 한다. 하부 &lt;Route&gt; 들을 상위 Nesting Auth Route의 자식으로 디스패치하는 작업도 필요하다. (코드 참고)
5. 기존의 &lt;Routes&gt;도 사용가능하다. 접근제어가 적용되지 않는 URL에 매핑된 컴포넌트들의 컨테이너가 된다.

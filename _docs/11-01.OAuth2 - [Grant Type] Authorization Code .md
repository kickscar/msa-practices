...
2. 인가서버(Authorization Server)의 이 인가에 대한 승인(Grant)도 필요하기 때문에 클라이언트의 사용자의 권한 인가 엔드포인트는 인가서버의 인가 엔드포인트(Authorization Endpoint, authorization-uri)로 리다이렉트 된다.

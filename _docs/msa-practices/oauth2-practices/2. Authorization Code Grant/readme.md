...
하지만, 앞의 6. OpenID Endpoint Configuration에서 확인 한 Token Endpoint 호출로만 인가 서버로 부터 Access Token 발급이 가능하다. 바로 OAuth2의 Authorization Grant Type 중에 Resource Owner Password Credential Grant Type의 플로우는 Token Endpoint 그리고 API 호출이 가능한 API 테스트 툴만 있으면 테스트 진행이 가능하다.
...
4. Security Configuration(com.poscodx.emaillist.security.Config.java)
	<p>
	OAuth2 Client 설정에 사용하는 HttpSecurity Security Builder의 DSL은 oauth2Login()와 oauth2Client() 이다. oauth2Client()는 OAuth2의 액세스 모델인 인가 승인 그리고 위임만 사용할 경우 고려해 볼 수 있다. 사실, OAuth2는 인증(Authetication) 프로토콜이 아니다.  인가(Authorization)에만 관심있는 프로토콜이다. 
	</p>
	<p>
	OAuth2의 인가 모델위에 자연스럽게 인증 모델이 통합된 것이 OpenID Connect 프로토콜이다. oauth2Client()는 OAuth2만 지원하기 때문에 Access Token 발급까지만 가능하며 oauth2Login()은 OIDC까지 지원하기 때문에 ID Token 발급도 가능하며 이를 통해 사용자 Identity까지 다룰 수 있다. 
	</p>
	...
	3) 접근제어를 하지 않는다.
	4) 사용자가 클라이언트를 인가하는 URL의 Base URL이다.
       따라서 인가 URL 형식은 /oauth2/authorize/{client-id} 가 된다.
	5) 인가 서버에서 사용자 인증이 성공하고 유효한 Client면 이 URL로 리다이렉트 한다.
       베이스 URL를 지정했는데 예제의 emaillist 클라이언트라면 /login/oauth2/code/emaillist
       에 파라미터로 인가 코드(Authorization Code)를 넘겨 줄것이다.
	6) Spring Security OAuth2는 5번 리다렉트 URL에 매핑된 핸들러에서 전달 받은
       Authorization Code를 가지고 다음 플로우를 진행하게 된다. 그리고 최종으로 인가 승인이
       완료되면 Bean 등록한 SuccessHandler에 인가 승인 토큰을 전달하면서 SuccessHandler가
       실행 된다. 한편, 특정 url로 리다이렉트 시켜 줄 수도 있는데 API 호출 클라이언트인 자바스크립트
       에게 토큰을 전달해야하는 로직 때문에 주석 처리했다. 이는 뒤에서 다시 설명한다.
	</pre>
5. Client Registration & Grant Provider (application.yml)
	<p>

...
export const AuthContextProvider = ({ children }) => {
    // 1. 새 access token 발급받음(반드시 동기 통신)
    syncFetchToken();

    // 2. 발급받은 새 access token을 Context State 으로 저장
    const tokenState = useState(ACCESSTOKEN);

    return (
        <AuthContext.Provider value={{ token: tokenState[0], storeToken: tokenState[1] }}>
            {children}
...
const syncFetchToken = () => {
    ...
    xhr.addEventListener('load', () => {
        ...
        if (!json.data) {
            // 1. cookie에 refresh token이 없거나(after logout 또는 first start)
            // 2. 또는 기간이 만료,
            // 3. 또는 유효 않은 refresh token를 cookie로 보냈음.
    ...
    xhr.open('get', '/api/user/refresh-token', false);

    ...
    return async (param) => {
        try {
            ...
            if(response.status === 401 && authorized) { // Unauthorized (Invalid or Expired Token)!
                response = await fetch(process.env.REFRESH_TOKEN_ENDPOINT, {method: 'get', headers: {'Accept': 'application/json', credentials: 'include'}}); 
...
const syncFetchToken = () => {
    ...
    xhr.open('get', process.env.REFRESH_TOKEN_ENDPOINT, false);

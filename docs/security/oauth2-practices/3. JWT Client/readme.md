...
6. js가 접근하지 못하지만 서버로 전달되는 http 헤더를 통해 refresh token이 탈취될 가능성이 남아 있기 때문에 ssl(https)로 모든 http 스택을 암호화 한다. 

#### React Context
1. access token의 payload에 보통 애플리케이션에서 필요한 기본적인 사용자 정보(no, username, email, profile 등)를 담는다.
